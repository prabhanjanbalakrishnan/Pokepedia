...
- `japaneseName`, `classification` (e.g. "Seed Pokémon"), `abilities` (`[{ name, hidden }]`), `weightKg`, `speed` — pulled straight from PokeAPI's `/pokemon-species/{id}` (`names` filtered to `language.name === 'ja'`, `genera` filtered to `'en'`) and `/pokemon/{id}` (`abilities`, `weight` in hectograms÷10, and the `speed` entry of `stats`) via `extract_details()`. Ability slugs (e.g. `"lightning-rod"`) are title-cased for display (`format_ability_name`), not fetched from PokeAPI's own localized ability names, to avoid an extra ~800 requests. This was the first time `build_data.py` fetched the `/pokemon/{id}` endpoint in addition to `/pokemon-species/{id}` — cached as `pokemon_{id}.json` alongside the existing `species_{id}.json`/`chain_{id}.json` cache files.
