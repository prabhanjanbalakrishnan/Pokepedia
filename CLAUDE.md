...
- `japaneseName`, `classification` (e.g. "Seed Pokémon"), `abilities` (`[{ name, hidden }]`), `heightM`, `weightKg`, `attack`, `defense`, `specialAttack`, `specialDefense`, `speed` — pulled straight from PokeAPI's `/pokemon-species/{id}` (`names` filtered to `language.name === 'ja'`, `genera` filtered to `'en'`) and `/pokemon/{id}` (`abilities`; `height`/`weight` in decimeters/hectograms÷10; the five non-HP entries of `stats`) via `extract_details()`. Ability slugs (e.g. `"lightning-rod"`) are title-cased for display (`format_ability_name`), not fetched from PokeAPI's own localized ability names, to avoid an extra ~800 requests. This was the first time `build_data.py` fetched the `/pokemon/{id}` endpoint in addition to `/pokemon-species/{id}` — cached as `pokemon_{id}.json` alongside the existing `species_{id}.json`/`chain_{id}.json` cache files. The five base stats render as bars scaled against a fixed `STAT_MAX = 200` (not each Pokemon's own max) so bar lengths are comparable across the whole detail page — a stat above 200 (rare; e.g. some legendaries) just fills the bar completely rather than overflowing it.
